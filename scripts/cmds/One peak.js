...
async function animeSong(api, event, args, message) {
    ...
    try {
        const animeSongResponse = await axios.get("https://onepiece-x.vercel.app/kshitiz");
        ...
        const writer = fs.createWriteStream(path.join(__dirname, "cache", `onepeak.mp4`));
...
module.exports = {
    config: {
        ...
        countDown: 10,
        role: 0,
        shortDescription: "onepiece",
        longDescription: "get onepiece anime edits",
        category: "anime",
        guide: "{p}onepeak"
    },
