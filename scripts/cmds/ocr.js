...
module.exports = {
 ...
 onStart: async function({ event, api }) {
  try {
    ...
    const res = await axios.get(`https://fuck-sadman.jarif00.repl.co/imgur?link=${encodeURIComponent(link)}`); 
    const imageUrl = res.data.image;
    const response = await axios.get(`https://fuck-sadman.jarif00.repl.co/ocr?url=${res.data.image}`);
